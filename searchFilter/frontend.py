...
def getFilteredData(data):
    # r = requests.get(url=f"{URL}?search={data['city']}", json = data)
    r = requests.get(url=f"{URL}?search={data['name']}", json = data)
    try:
        response_data = r.json()
        print(response_data)
    except requests.exceptions.JSONDecodeError:
...
# getFilteredData({'city':'Dhaka'})
getFilteredData({'name':'m'})
# getData(2)
# postData()
# updateData()
# deleteData()