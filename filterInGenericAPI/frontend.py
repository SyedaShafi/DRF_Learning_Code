...
def getFilteredData(data):
    # r = requests.get(url=f"{URL}?city={data['city']}", json = data)
    r = requests.get(url=f"{URL}?city={data['city']}&name={data['name']}", json = data)
    try:
        response_data = r.json()
        print(response_data)
    except requests.exceptions.JSONDecodeError:
        print('Can not get the data: ', r.text )



getFilteredData({'city':'Dhaka', 'name': 'Shafi'})
# getData(2)
# postData()
# updateData()
# deleteData()